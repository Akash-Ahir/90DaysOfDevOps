...
### 6) " /var→ Variable Files "
  - it contains hardware device filesContains device files
...
### 9) " /usr→ User Binaries "
   - The /usr directory contains applications and files used by users, as opposed toapplications and files used by the system.
...
Note down the pid of the process which has the higest cpu usage
...
```
Confirms service exists and their status 
